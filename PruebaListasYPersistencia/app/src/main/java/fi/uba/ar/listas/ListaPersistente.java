...
public class ListaPersistente <T extends ElementoListaPersistente> {

    private ConfigTabla _configTabla;
    private BaseDeDatosHelper _bdHelper;

    private ListaPersistente(Context context)
    {
        _bdHelper = BaseDeDatosHelper.crearHelper(context, new ConfigArmarioBD());
    }

    public static ListaPersistente crearLista(Context context)
    {
        return new ListaPersistente<Prenda>(context);
    }
